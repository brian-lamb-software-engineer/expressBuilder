...
gulp.task('client-js', ['coffee-js'], function(){
  ...
  var jsModels = gulp.src(config.paths.models + '*.js')
    .pipe(jshint())
    // .pipe(uglify()) //breaks, why?
    .pipe(gulp.dest(config.paths.pub + 'js/'));

  // services, loaded separately
  var jsServices = gulp.src(config.paths.services + '*.js')
    .pipe(jshint())
    // .pipe(uglify()) //breaks, why?
    .pipe(gulp.dest(config.paths.pub + 'js/'));

  // controllers, loaded separately from routing
  var jsControllers = gulp.src(config.paths.controllers + '*.js')
    .pipe(jshint())
    // .pipe(uglify()) //breaks, why?
    .pipe(gulp.dest(config.paths.pub + 'js/'));
  ...
  var srcFootJs = gulp.src([config.paths.assets + 'js/dummy.js'])
    .pipe(jshint())
...
gulp.task('jasmine-tests', function () {
    return gulp.src(config.paths.tests + 'tests.js')
        .pipe(jasmine());
});
